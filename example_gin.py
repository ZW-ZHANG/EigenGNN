# adapted from https://github.com/PurdueMINDS/RelationalPooling/blob/master/Run_Gin_Experiment.py
# make sure you can run the original script and put example_gin.py and EigenGNN.py in the same folder before running the code
# example usage: python example_gin.py --cv-fold 0 --model-type EigenGin --num-mlp-hidden 1 --num-gnn-layers 1 --out-weight-dir dir1 --out-log-dir dir2
# EigenGNN is mainly applied in line 224-229 

"""
Synthetic experiments with GIN and RP-GIN

Usage:
    example_gin.py (--cv-fold <N>) (--out-weight-dir <folder>) (--out-log-dir <folder>) [--use-batchnorm] [--dense-dropout-prob <float>]
...
Options:
    ...
    --data-per-class <N>            Number of instances for each class [default: 60]
    --eigd <N>                      Dimensionality of eigenvectors [default: 16]
"""
...
def data_augment(num, adj_list, y_list, seed = 0):   # randomly permutate adj_list to generate isomorphic graphs
    ...
    for i in range(num - 1):
        adj_list = adj_list + copy.deepcopy(adj_list_copy)
        y_list = torch.cat((y_list,y_list_copy))
    np.random.seed(seed)
    n_nodes = adj_list[0].shape[0]
    for j in range(len(adj_list)):
        temp_perm = np.random.permutation(n_nodes)
        perm_mat = sp.csr_matrix((np.ones(n_nodes),(np.array(range(n_nodes)),temp_perm)),(n_nodes,n_nodes))
        adj_list[j] = sp.coo_matrix(perm_mat * adj_list[j] * perm_mat.T)
    return adj_list, y_list   
...
if __name__ == '__main__':
    requirements = {
        ...
        '--out-log-dir': Use(str),
        '--model-type': And(Use(str), lambda sss: sss in ['regularGin', 'dataAugGin', 'rpGin', 'EigenGin']),
    ...
    if args['--model-type'] == 'regularGin'  or args['--model-type'] == 'EigenGin':
